...
interface Props extends PanelProps<CalendarioOptions> {}

export const CalendarioPanel: React.FC<Props> = ({ options, data, width, height, fieldConfig, id, replaceVariables  }) => {
    ...
    <RangePicker
      ...
      onChange={value => {
        locationService.partial({
          [`var-${options.variable_data_inizio}`]: value?.[0]?.format('YYYY-MM-DD') ?? undefined,
          [`var-${options.variable_data_fine}`]: value?.[1]?.format('YYYY-MM-DD') ?? undefined
        }, true)}
      }
    />
  );
